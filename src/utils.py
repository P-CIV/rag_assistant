...
# Variables globales pour les moteurs (singleton)
_moteur_chat = None
_moteur_embedding = None


def creer_modele_chat():
    """Retourne le moteur de chat Azure OpenAI (singleton)."""
    global _moteur_chat
    if _moteur_chat is None:
        if not AZURE_CHAT_API_KEY or not AZURE_CHAT_ENDPOINT:
            raise ValueError(
                "Credentials Azure OpenAI Chat manquantes. Vérifie ton .env"
            )
...
def creer_modele_embedding():
    """Retourne le moteur d'embedding Azure OpenAI (singleton)."""
    global _moteur_embedding
    if _moteur_embedding is None:
        if not AZURE_EMBEDDING_API_KEY or not AZURE_EMBEDDING_ENDPOINT:
            raise ValueError(
                "Credentials Azure OpenAI Embedding manquantes. Vérifie ton .env"
            )
...
def lire_prompt(chemin_fichier: str) -> str:
    """Lit et retourne le contenu du fichier prompt"""
...
def formater_documents(documents: list) -> str:
    """Formate les documents en texte structuré pour le prompt"""
    if not documents:
    ...
    for i, doc in enumerate(documents, 1):
        ...
        if page is not None:
            ref_source = f"{nom_fichier}, page {page + 1}"
        else:
            ref_source = f"{nom_fichier}, document complet"

        parties.append(f"[Document {i} — {ref_source}]\n{doc.page_content}")
